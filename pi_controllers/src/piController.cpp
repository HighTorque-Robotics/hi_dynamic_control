...
#include "pi_controllers/piController.h"
...
#include <pi_estimation/FromTopiceEstimate.h>
#include <pi_estimation/LinearKalmanFilter.h>
#include <pi_wbc/WeightedWbc.h>
#include <pluginlib/class_list_macros.hpp>
#include "std_msgs/Float64MultiArray.h"
#include "pi_controllers/utilities.h"
...
namespace legged
{
bool piController::init(hardware_interface::RobotHW* robot_hw, ros::NodeHandle& controller_nh)
{
  // Initialize OCS2
  std::string urdfFile;
  std::string taskFile;
  ...
  // Visualization
  ros::NodeHandle nh;
  CentroidalModelPinocchioMapping pinocchioMapping(PiInterface_->getCentroidalModelInfo());
  eeKinematicsPtr_ = std::make_shared<PinocchioEndEffectorKinematics>(
      PiInterface_->getPinocchioInterface(), pinocchioMapping, PiInterface_->modelSettings().contactNames3DoF);
  robotVisualizer_ = std::make_shared<piRobotVisualizer>(
      PiInterface_->getPinocchioInterface(), PiInterface_->getCentroidalModelInfo(), *eeKinematicsPtr_, nh);
  selfCollisionVisualization_.reset(new piSelfCollisionVisualization(
      PiInterface_->getPinocchioInterface(), PiInterface_->getGeometryInterface(), pinocchioMapping, nh));
  ...
  rbdConversions_ = std::make_shared<CentroidalModelRbdConversions>(PiInterface_->getPinocchioInterface(),
                                                                    PiInterface_->getCentroidalModelInfo());
  ...
  // Whole body control
  wbc_ = std::make_shared<WeightedWbc>(PiInterface_->getPinocchioInterface(),
                                       PiInterface_->getCentroidalModelInfo(), *eeKinematicsPtr_, controller_nh);
  wbc_->loadTasksSetting(taskFile, verbose);
  wbc_->setStanceMode(true);

  // Safety Checker
  safetyChecker_ = std::make_shared<SafetyChecker>(PiInterface_->getCentroidalModelInfo());

  // Configuring the hardware interface
  eeKinematicsPtr_->setPinocchioInterface(PiInterface_->getPinocchioInterface());
  ...
  inverseKinematics_.setParam(std::make_shared<PinocchioInterface>(PiInterface_->getPinocchioInterface()),
                              std::make_shared<CentroidalModelInfo>(PiInterface_->getCentroidalModelInfo()));

  info_ = PiInterface_->getCentroidalModelInfo();

  return true;
}

void piController::starting(const ros::Time& time)
{
  ...
  // Dynamic server
  serverPtr_ =
      std::make_unique<dynamic_reconfigure::Server<pi_controllers::TutorialsConfig>>(ros::NodeHandle("controller"));
  dynamic_reconfigure::Server<pi_controllers::TutorialsConfig>::CallbackType f;
  f = boost::bind(&piController::dynamicParamCallback, this, _1, _2);
  serverPtr_->setCallback(f);
}

void piController::update(const ros::Time& time, const ros::Duration& period)
{
  const ros::Time shifted_time = time - startingTime_;
  // State Estimate
  ...
  posDes_ = centroidal_model::getJointAngles(optimizedState, PiInterface_->getCentroidalModelInfo());
  velDes_ = centroidal_model::getJointVelocities(optimizedInput, PiInterface_->getCentroidalModelInfo());
  ...
  {
    //"Limit protection
    const auto& model = PiInterface_->getPinocchioInterface().getModel();
  ...
  robotVisualizer_->update(currentObservation_, primal_solution, command_data,
                           PiInterface_->getSwitchedModelReferenceManagerPtr()->getSwingTrajectoryPlanner());
...
void piController::updateStateEstimation(const ros::Time& time, const ros::Duration& period)
{
  ...
  stateEstimate_->setStartStopTime4Legs(
      PiInterface_->getSwitchedModelReferenceManagerPtr()->getSwingTrajectoryPlanner()->threadSaftyGetStartStopTime(
  ...
  const auto& reference_manager = PiInterface_->getSwitchedModelReferenceManagerPtr();
...
void piController::setupLeggedInterface(const std::string& taskFile, const std::string& urdfFile,
                                            const std::string& referenceFile, bool verbose)
{
  PiInterface_ = std::make_shared<piInterface>(taskFile, urdfFile, referenceFile);
  PiInterface_->setupOptimalControlProblem(taskFile, urdfFile, referenceFile, verbose);
}

void piController::setupMpc()
{
  mpc_ = std::make_shared<SqpMpc>(PiInterface_->mpcSettings(), PiInterface_->sqpSettings(),
                                  PiInterface_->getOptimalControlProblem(), PiInterface_->getInitializer());

  const std::string robotName = "legged_robot";
  ros::NodeHandle nh;
  auto rosReferenceManagerPtr =
      std::make_shared<RosReferenceManager>(robotName, PiInterface_->getReferenceManagerPtr());
  rosReferenceManagerPtr->subscribe(nh);
  mpc_->getSolverPtr()->setReferenceManager(rosReferenceManagerPtr);
  observationPublisher_ = nh.advertise<ocs2_msgs::mpc_observation>(robotName + "_mpc_observation", 1);
}

void piController::setupMrt()
{
  mpcMrtInterface_ = std::make_shared<MPC_MRT_Interface>(*mpc_);
  mpcMrtInterface_->initRollout(&PiInterface_->getRollout());
  mpcTimer_.reset();
    ...
    while (controllerRunning_)
    {
      try
      {
        executeAndSleep(
            ...
            PiInterface_->mpcSettings().mpcDesiredFrequency_);
      }
  ...
  setThreadPriority(PiInterface_->sqpSettings().threadPriority, mpcThread_);
}

void piController::setupStateEstimate(const std::string& taskFile, bool verbose)
{
  stateEstimate_ = std::make_shared<KalmanFilterEstimate>(
      PiInterface_->getPinocchioInterface(), PiInterface_->getCentroidalModelInfo(), *eeKinematicsPtr_);
  stateEstimate_->loadSettings(taskFile, verbose);
  dynamic_cast<KalmanFilterEstimate&>(*stateEstimate_).loadSettings(taskFile, verbose);
  currentObservation_.time = 0;
}

void piController::dynamicParamCallback(pi_controllers::TutorialsConfig& config, uint32_t level)
{
...
void piController::RetrievingParameters()
{
  stateDim_ = PiInterface_->getCentroidalModelInfo().stateDim;
  inputDim_ = PiInterface_->getCentroidalModelInfo().inputDim;
  jointDim_ = PiInterface_->getCentroidalModelInfo().actuatedDofNum;
  footDim_ = PiInterface_->getCentroidalModelInfo().numThreeDofContacts;
  gencoordDim_ = PiInterface_->getCentroidalModelInfo().generalizedCoordinatesNum;
  dofPerLeg_ = jointDim_ / 2;
  defalutJointPos_.resize(jointDim_);
}

void piController::resetMPC()
{
  TargetTrajectories target_trajectories({ currentObservation_.time }, { currentObservation_.state },
                                         { currentObservation_.input });
  mpcMrtInterface_->resetMpcNode(target_trajectories);
}
void piController::ModeSubscribe()
{
  subSetWalk_ =
      ros::NodeHandle().subscribe<std_msgs::Float32>("/set_walk", 1, &piController::setWalkCallback, this);
  subLoadcontroller_ = ros::NodeHandle().subscribe<std_msgs::Float32>("/load_controller", 1,
                                                                      &piController::loadControllerCallback, this);
  subEmgstop_ = ros::NodeHandle().subscribe<std_msgs::Float32>("/emergency_stop", 1,
                                                               &piController::EmergencyStopCallback, this);
  subResetTarget_ = ros::NodeHandle().subscribe<std_msgs::Float32>("/reset_estimation", 1,
                                                                  &piController::ResetTargetCallback, this);                                                               
}

void piController::EmergencyStopCallback(const std_msgs::Float32::ConstPtr& msg)
{
  emergencyStopFlag_ = true;
  ROS_INFO("unload the controller");
}

void piController::setWalkCallback(const std_msgs::Float32::ConstPtr& msg)
{
  setWalkFlag_ = true;
  ROS_INFO("Set WALK Mode");
}

void piController::loadControllerCallback(const std_msgs::Float32::ConstPtr& msg)
{
...
void piController::ResetTargetCallback(const std_msgs::Float32::ConstPtr& msg)
{
    // Initial state
  currentObservation_.state.setZero(stateDim_);
  currentObservation_.input.setZero(inputDim_);
...
PLUGINLIB_EXPORT_CLASS(legged::piController, controller_interface::ControllerBase)