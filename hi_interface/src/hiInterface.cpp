...
#include "hi_interface/constraint/FrictionConeConstraint.h"
#include "hi_interface/constraint/LeggedSelfCollisionConstraint.h"
#include "hi_interface/constraint/NormalVelocityConstraintCppAd.h"
#include "hi_interface/constraint/XYReferenceConstraintCppAd.h"
#include "hi_interface/constraint/XYLimitConstraint.h"
#include "hi_interface/constraint/ZeroForceConstraint.h"
#include "hi_interface/constraint/ZeroVelocityConstraintCppAd.h"
#include "hi_interface/cost/hiRobotQuadraticTrackingCost.h"
#include "hi_interface/initialization/hiRobotInitializer.h"
...
#include "hi_interface/constraint/FootRollConstraint.h"
#include <hi_interface/dynamics/hiRobotDynamicsAD.h>
...
namespace legged
{
hiInterface::hiInterface(const std::string& taskFile, const std::string& urdfFile,
  ...
  : useHardFrictionConeConstraint_(useHardFrictionConeConstraint)
{
  ...
  {
    std::cerr << "[hiInterface] Loading task file: " << taskFilePath << std::endl;
  }
  else
  {
    throw std::invalid_argument("[hiInterface] Task file not found: " + taskFilePath.string());
  }
  ...
  {
    std::cerr << "[hiInterface] Loading Pinocchio model from: " << urdfFilePath << std::endl;
  }
  else
  {
    throw std::invalid_argument("[hiInterface] URDF file not found: " + urdfFilePath.string());
  }
  ...
  {
    std::cerr << "[hiInterface] Loading target command settings from: " << referenceFilePath << std::endl;
  }
  else
  {
    throw std::invalid_argument("[hiInterface] targetCommand file not found: " + referenceFilePath.string());
  }
...
/******************************************************************************************************/
/******************************************************************************************************/
void hiInterface::setupOptimalControlProblem(const std::string& taskFile, const std::string& urdfFile,
...
hiInterface::getSoftSwingTrajConstraint(const EndEffectorKinematics<scalar_t>& eeKinematics,
...
// std::unique_ptr<StateInputConstraint> hiInterface::getFootRollConstraint(size_t contactPointIndex){
...
/******************************************************************************************************/
/******************************************************************************************************/
void hiInterface::setupModel(const std::string& taskFile, const std::string& urdfFile,
                                 const std::string& referenceFile, bool /*verbose*/)
{
  // PinocchioInterface
...
/******************************************************************************************************/
/******************************************************************************************************/
void hiInterface::setupReferenceManager(const std::string& taskFile, const std::string& urdfFile,
                                            const std::string& referenceFile, bool verbose)
{
...
/******************************************************************************************************/
/******************************************************************************************************/
void hiInterface::setupPreComputation(const std::string& taskFile, const std::string& urdfFile,
                                          const std::string& referenceFile, bool verbose)
{
...
/******************************************************************************************************/
/******************************************************************************************************/
std::shared_ptr<GaitSchedule> hiInterface::loadGaitSchedule(const std::string& file, bool verbose) const
{
...
/******************************************************************************************************/
/******************************************************************************************************/
matrix_t hiInterface::initializeInputCostWeight(const std::string& taskFile, const CentroidalModelInfo& info)
{
...
/******************************************************************************************************/
/******************************************************************************************************/
std::unique_ptr<StateInputCost> hiInterface::getBaseTrackingCost(const std::string& taskFile,
...
/******************************************************************************************************/
/******************************************************************************************************/
std::unique_ptr<StateInputCost> hiInterface::getLimitConstraints(const CentroidalModelInfo& info)
{
  const size_t totalContactDim = 3 * info.numThreeDofContacts + 6 * info.numSixDofContacts;
...
hiInterface::loadFrictionConeSettings(const std::string& taskFile, bool verbose)
{
  boost::property_tree::ptree pt;
  boost::property_tree::read_info(taskFile, pt);
...
/******************************************************************************************************/
/******************************************************************************************************/
std::unique_ptr<StateInputConstraint> hiInterface::getFrictionConeConstraint(size_t contactPointIndex,
...
/******************************************************************************************************/
/******************************************************************************************************/
std::unique_ptr<StateInputCost> hiInterface::getFrictionConeSoftConstraint(
...
hiInterface::getEeKinematicsPtr(const std::vector<std::string>& footNames, const std::string& modelName)
{
...
/******************************************************************************************************/
/******************************************************************************************************/
std::unique_ptr<StateInputConstraint> hiInterface::getZeroVelocityConstraint(
...
/******************************************************************************************************/
/******************************************************************************************************/
std::unique_ptr<StateCost> hiInterface::getSelfCollisionConstraint(const PinocchioInterface& pinocchioInterface,
